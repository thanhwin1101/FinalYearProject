// ====================================================================
//  carry_final  –  ESP32 Slave  –  main.cpp
//  UART<->ESP32 Master, Mecanum logic, line following, NFC, SR05
// ====================================================================
#include <Arduino.h>
#include <WiFi.h>
#include "config.h"
    ...
    while (uartReceiveFrame(SerialMaster, cmd, buf, len)) {
        ...
        case CMD_SET_MODE:
            if (len >= 1) {
                ...
                // ACK
                uint8_t ack = cmd;
                uartSendFrame(SerialMaster, CMD_ACK, &ack, 1);
            }
            break;
        ...
        case CMD_REQUEST_STATUS:
            ...
            {
                uint8_t pct = 100;
                uartSendFrame(SerialMaster, CMD_BATTERY, &pct, 1);
            }
            break;
...
void setup() {
    Serial.begin(115200);
    Serial.println("\n=== CarryFinal ESP32 Slave ===");

    // UART to ESP32 master (Serial1)
    SerialMaster.begin(ESP_BAUD, SERIAL_8N1, PIN_UART_RX, PIN_UART_TX);

    // UART to HuskyLens (Serial2)
...
void loop() {
    if (WiFi.status() == WL_CONNECTED) otaLoop();
    handleESP32();
    ...
    // ── Periodic line sensor status → ESP32 (every 80ms) ────────────
    {
        ...
        if (now - lastLineSend >= 80) {
            ...
            uint8_t bits = (lineLeft()   ? 0x01 : 0)
                         | (lineCenter() ? 0x02 : 0)
                         | (lineRight()  ? 0x04 : 0);
            uartSendFrame(SerialMaster, CMD_LINE_STATUS, &bits, 1);
        }
    }
        ...
        case MODE_AUTO:
            ...
            if (!g_missionRunning) {
                ...
                if (nfcId != 0) {
                    ...
                    uartSendFrame(SerialMaster, CMD_CHECKPOINT, buf, 2);
