...
static void reportHuskyStatus(const HuskyResult &r) {
    ...
    uartSendFrame(Serial2, CMD_HUSKY_STATUS, buf, 11);
}

static void reportTagLost() {
    uartSendFrame(Serial2, CMD_TAG_LOST, nullptr, 0);
}

static void reportTagFound() {
    uartSendFrame(Serial2, CMD_TAG_FOUND, nullptr, 0);
}
...
void followRunnerLoop() {
    ...
    if (tag.detected) {        if (s_tagLostReported) {
        ...
        // tag not visible
        if (!s_tagLostReported && (now - s_lastTagTime > TAG_LOST_MS)) {
            motorStop();
            reportTagLost();
            s_tagLostReported = true;
            Serial.println("[FOLLOW-STM] tag lost â†’ reported");
        }
    }
